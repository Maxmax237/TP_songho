<?php
// api.php pour Render.com - Version simplifiée
...
$dbname = getenv('PGDATABASE') ?: 'songho_online';
...
switch ($action) {
    ...
    case 'reset_room':
        ...
        $state = getInitialState();
        saveGameState($pdo, $roomCode, $state);
        $stmt = $pdo->prepare("UPDATE game_rooms SET current_player = 'south', game_active = true, game_ended = false, winner = NULL WHERE room_code = ?");
        $stmt->execute([$roomCode]);
