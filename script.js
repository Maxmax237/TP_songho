...
function renderBoard() {
    ...
    for (let i = 0; i < gameState.boardNorth.length; i++) {
        ...
        const seedsCount = gameState.boardNorth[i];
        iconSpan.innerText = '●'.repeat(Math.min(seedsCount, 15));
    ...
    for (let i = 0; i < gameState.boardSouth.length; i++) {
        ...
        const iconSpan = document.createElement('div');
        iconSpan.className = 'seed-icon';
        const seedsCount = gameState.boardSouth[i];
        iconSpan.innerText = '●'.repeat(Math.min(seedsCount, 15));
...
async function loadGameState() {
    ...
    if (result && result.success) {
        ...
        const oldGameEnded = gameState?.gameEnded;
        gameState = result.state;
        renderBoard();
        
        if (oldGameEnded !== gameState.gameEnded && gameState.gameEnded) {
            gameMessage.innerText = gameState.winner || 'Partie terminée !';
        } else if (oldTurn !== gameState.currentPlayer && gameState.gameActive && !gameState.gameEnded) {
            if (playerRole === gameState.currentPlayer) {
...
async function createGame() {
    ...
    if (result && result.success) {
        ...
        renderBoard();
        startPolling();
        gameMessage.innerText = result.message || 'En attente du deuxième joueur...';
    } else {
...
async function joinGame() {
    ...
    if (result && result.success) {
        ...
        renderBoard();
        startPolling();
        gameMessage.innerText = result.message || 'Partie rejointe ! Attendez votre tour...';
    } else {
...
// Démarrer le polling au chargement (juste pour afficher)
